...
class BuildModelStack(cdk.Stack):
    def __init__(
        ...
        **kwargs,
        ...
        sm_role = iam.Role.from_role_arn(
            self, "SMRole", role_arn=sagemaker_execution_role_arn
        )
        ...
        for k in configuration_path.glob("*.pipeline.json"):
            ...
            try:
                ...
                # TODO: build and push customized docker image to ECR
                
                if "containers" in pipeline_conf:
                    for image_name, env in pipeline_conf["containers"].items():
                        asset = ecr.DockerImageAsset(self,
                                                    image_name, 
                                                    directory=env["docker_dir"],
                                                    repository_name=env["repo_name"])
                        
                        pipeline_conf['containers'][image_name] = asset.image_uri
