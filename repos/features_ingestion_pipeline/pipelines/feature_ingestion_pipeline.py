...
def create_pipeline(role, pipeline_name, sagemaker_session=None, **kwarg)-> Pipeline:
    ...
    data_sources = []
    input_data_urls = []
    for input_name in flow_file.input_name:
        file_type = input_name.split('.')[0]
        input_data_url = ParameterString(name=f"InputDataUrl_{file_type}")
        ...
        input_data_urls.append(input_data_url)
    ...
    pipeline = Pipeline(
        name=pipeline_name,
        parameters=[instance_count, instance_type]+input_data_urls,
